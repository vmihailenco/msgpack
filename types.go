...
func getFields(typ reflect.Type, useJSONTag bool, parentTag string) *fields {
	...
	for i := 0; i < numField; i++ {
		...
		if f.Anonymous && inlineFields(fs, f.Type, field, useJSONTag, field.name) {
			continue
		}
...
func inlineFields(fs *fields, typ reflect.Type, f *field, useJSONTag bool, parentTag string) bool {
	...
	inlinedFields := getFields(typ, useJSONTag, parentTag).List
