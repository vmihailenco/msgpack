...
func decodeCustomTestStruct(d *msgpack.Decoder, v reflect.Value) error {
	...
	if err = d.Decode(&o.s, &o.c, &o.n); err == nil {
...
func TestCustom_Structs(t *testing.T) {
	...
	tests := []struct {
		...
		{&customTestStruct{"n", &customTestStruct{"o", nil, 7}, 6},
			[]byte{0xa1, 'n', 0xa1, 'o', nilCode, 0x7, 0x6}},
		{&customTestStruct{},
			[]byte{0xa0, nilCode, 0x0}},
		{nil,
			[]byte{nilCode}},
	}
